...
def page_dashboard_and_register(df: pd.DataFrame, settings: Dict[str, Any], storage_mode: str) -> None:
    ...
    with left:
        ...
        reset_token = st.session_state.get("register_reset_token", 0)
        ...
        with st.container():
            c1, c2 = st.columns(2)
            with c1:
                fridge = st.selectbox("보관 장소", fridge_list + ["직접 입력"], index=0, key=f"reg_fridge_{reset_token}")
                fridge_manual = ""
                if fridge == "직접 입력":
                    fridge_manual = st.text_input("새 보관 장소", key=f"reg_fridge_manual_{reset_token}")
            with c2:
                category = st.selectbox("보관 유형", category_list + ["직접 입력"], index=0, key=f"reg_category_{reset_token}")
                category_manual = ""
                if category == "직접 입력":
                    category_manual = st.text_input("새 보관 유형", key=f"reg_category_manual_{reset_token}")

            name = st.text_input("재고명", placeholder="예: 닭가슴살, 계란, 양파", key=f"reg_name_{reset_token}")
            amount_mode = st.radio("수량 입력 방식", ["무게(g)로 입력", "개수(개)로 입력"], horizontal=True, key=f"reg_amount_mode_{reset_token}")
            ...
            with c3:
                if amount_mode == "무게(g)로 입력":
                    ...
                    total_weight = st.number_input("총무게(g)", min_value=0.0, value=default_weight, step=10.0, key=f"reg_total_weight_{reset_token}")
                    total_quantity = 0
                else:
                    default_quantity = max(1, safe_int(settings.get("default_quantity", "1"), 1))
                    total_quantity = st.number_input("총개수(개)", min_value=1, value=default_quantity, step=1, key=f"reg_total_quantity_{reset_token}")
                    total_weight = 0.0
            with c4:
                total_price = st.number_input("총가격(원)", min_value=0, value=0, step=100, key=f"reg_total_price_{reset_token}")
            with c5:
                default_count = max(1, safe_int(settings.get("default_count", "1"), 1))
                count = st.number_input("나눌 묶음 수", min_value=1, value=default_count, step=1, key=f"reg_count_{reset_token}")
            if amount_mode == "개수(개)로 입력":
                st.caption("예: 계란 10개를 한 줄로 저장하려면 총개수 10, 나눌 묶음 수 1로 입력하세요. 10개를 각각 나누려면 나눌 묶음 수 10으로 입력하세요.")

            expiry_mode = st.radio("유통기한 입력 방식", ["며칠 뒤 만료", "날짜 선택", "없음"], horizontal=True, key=f"reg_expiry_mode_{reset_token}")
            expiry_date: Optional[date] = None
            if expiry_mode == "며칠 뒤 만료":
                default_days = safe_int(settings.get("default_expiry_days", ""), 0)
                days = st.number_input("기한(일)", min_value=0, value=default_days, step=1, key=f"reg_expiry_days_{reset_token}")
                expiry_date = today_date() + timedelta(days=int(days))
            elif expiry_mode == "날짜 선택":
                expiry_date = st.date_input("만료예정일", value=today_date(), key=f"reg_expiry_date_{reset_token}")

            detailed = False
            portion_amounts: List[float] = []
            if count > 1:
                detailed_label = "상세 소분: 조각별 무게를 직접 입력" if amount_mode == "무게(g)로 입력" else "상세 소분: 묶음별 개수를 직접 입력"
                detailed = st.checkbox(detailed_label, key=f"reg_detailed_{reset_token}")
                if detailed:
                    cols = st.columns(2)
                    if amount_mode == "무게(g)로 입력":
                            ...
                            with cols[i % 2]:
                                portion_amounts.append(
                                    st.number_input(
                                        ...
                                        step=10.0,
                                        key=f"detail_weight_{reset_token}_{i}",
                                    )
                                )
                                ...
                                portion_amounts.append(
                                    float(
                                        st.number_input(
                                            ...
                                            step=1,
                                            key=f"detail_quantity_{reset_token}_{i}",
                                        )
                                    )
                                )
            ...
            memo = st.text_input("메모", placeholder="선택 입력", key=f"reg_memo_{reset_token}")
            submitted = st.button("식재료 등록하기", use_container_width=True, key=f"reg_submit_{reset_token}")
        ...
        if submitted:
            ...
            if not name.strip():
                ...
                # 일반 container 기반 입력 폼은 위젯 상태가 남을 수 있으므로
                # 등록 성공 시 token을 바꿔 새 입력 위젯으로 다시 그립니다.
                st.session_state["register_reset_token"] = reset_token + 1
                st.rerun()
